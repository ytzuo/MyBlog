...
校内通用消息通知接口属于第三方接口，如果直接放在业务逻辑中调用，可能导致主流程响应变慢，所以我们设计了这样的事件流程：
...
原本测试中运行良好，可以正确地发送通知。
昨晚，根据心理中心老师的意见，我们修改了部分业务逻辑。由于时间较紧、人手不足，我们使用 `AI Coding` 修改代码，再由人工进行测试和 `Code Review`。
...
观察发现，这些记录没有进入过 `SENDING` 等状态，说明这些记录根本没有被分发，而是在落库那一刻就“死”了！
