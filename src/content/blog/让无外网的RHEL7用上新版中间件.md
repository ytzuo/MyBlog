...
因为新项目甲方不想要走学校的（又臭又长的）流程，所以为了后续部署，我们开始搜罗手上的服务器，发现了 4 个远古服务器：
...
古老的 `RHEL 7`（这套系统发布的时候我还在玩泥巴），但是我们使用的中间件——`PostgreSQL` 18 和 `Redis` 7.0——都不支持这么古老的系统。看看 PostgreSQL：

![PostgreSQL 支持情况](../../assets/BlogImg/SupportOfPostgreSQL.png)
...
由于我的设备是 `Windows`，所以采用 `Docker` 拉取 `CentOS 7` 镜像（因为同属红帽系），并进入镜像进行构建：
...
离线服务器无法访问外网，且 RHEL 7 系统较老，不适合强装新版 RPM。
